...
#'
#' @param opt_data tibble, same shape expected by [optimize_demand()].
...
#'   or the return value of [optimize_demand()] for the optimized cost.
#'
...
evaluate_cost <- function(opt_data, O) {
  ...
  PI <- if ("price_imported" %in% names(opt_data)) {
    opt_data$price_imported
  } else {
    1
  }
  PE <- if ("price_exported" %in% names(opt_data)) {
    opt_data$price_exported
  } else {
    0
  }
