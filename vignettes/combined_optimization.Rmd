...
Function `optimize_demand()` and `add_battery_optimization()` make use of the `HiGHS` optimization backend in order to obtain the optimal power load given certain conditions. Depending on the objective and the variables involved, the problem is solved as a quadratic program (QP), mixed-integer linear program (MILP) or mixed-integer quadratic program (MIQP). Currently, `flextools` package allows to optimize a time-series power load considering two different goals:
...
Moreover, this optimization problem has the constraints described in [Energy cost optimization article (demand section)](https://resourcefully-dev.github.io/flextools/articles/minimize_cost.html#demand-optimization), including the binary grid mode that prevents importing and exporting energy in the same time slot.
...
Moreover, this optimization problem has the constraints described in [Energy cost optimization article (battery section)](https://resourcefully-dev.github.io/flextools/articles/minimize_cost.html#battery-optimization), including the charging and discharging efficiencies ($\eta_c$, $\eta_d$), the binary battery mode that prevents simultaneous charging and discharging, and the binary grid mode that prevents simultaneous import and export.
