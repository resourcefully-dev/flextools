...
* Imported and exported energy can not be positive at the same time. This is enforced with one binary grid mode variable per time slot:

$$
0 \le I_t \le M^I_t y_t \qquad 0 \le E_t \le M^E_t (1-y_t) \qquad y_t \in \{0,1\} \quad t \in T
$$

Where $M^I_t$ and $M^E_t$ are finite upper bounds derived from the feasible import/export ranges at each time slot.
...
- $I_t$ : imported energy
- $E_t$ : exported energy
- $z_t$ : binary battery mode (1 = charging, 0 = discharging)
- $y_t$ : binary grid mode (1 = importing, 0 = exporting)
- $PI_t$ : imported energy price
- $PE_t$ : exported energy price
- $PTU_t$ : balancing price for turn-up power
...
$$
0 \le C_t \le B_c z_t \qquad 0 \le D_t \le B_d (1-z_t) \qquad z_t \in \{0,1\} \quad t \in T
$$
...
Where $M^I_t$ and $M^E_t$ are finite upper bounds derived from the feasible site import/export ranges at each time slot.
