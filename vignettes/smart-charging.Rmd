...
* **Flexible**: charging sessions that are flexible in a specific time slot that requires flexibility. This depends on the session's features and the scheduling `method` (as described in the [Scheduling algorithm](https://resourcefully-dev.github.io/flextools/articles/smart_charging.html#scheduling-algorithm) section). If no flexibility is required, then this variable will be `FALSE` even if they could provide flexibility.
