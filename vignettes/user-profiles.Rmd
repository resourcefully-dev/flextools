...
ev_sessions <- readr::read_csv("sessions_user_profiles.csv") %>% 
  ...
  filter(ConnectionStartDateTime <= dmy_hm("06/08/2024 07:00", tz = "Europe/Amsterdam"))
```
...
```{r}
plot(sc_results, ev_sessions, by = "FlexType")
```



We see that now the participation of EV users is not enough to reach our objective. For a more in-depth analysis we can use function `summarise_smart_charging_sessions()`, which shows the percentage of Considered, Responsive, Flexible and Exploited sessions:
...
* The not considered sessions correspond to the 7th August, a day that it's not in the optimization windows
* The Responsive percentages correspond to the 30% of Visit sessions and 60% of Worktime sessions
...
If we increase the participation of Worktime users up to 90% then we accomplish with our grid capacity again:
