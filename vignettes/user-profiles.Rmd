...
```{r setup, echo=FALSE, message=F, warning=FALSE, fig.width=9}
library(flextools)
library(dplyr)
library(lubridate)
library(dygraphs)
...
```{r, echo=FALSE}
ev_demand %>% 
  timefully::plot_ts(ylab = "Power demand (kW)", stackedGraph = T, fillGraph = T)
```
...
ev_demand_opt %>% 
  ...
  timefully::plot_ts(ylab = "Power demand (kW)", stackedGraph = T, fillGraph = T, legend_width = 150) %>% 
  dyLimit(grid_capacity, "Grid capacity", color = "red")
```
